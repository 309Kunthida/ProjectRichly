...
const AddTransaction = () => {
    ...
    const handleSubmit = async () => {
        if (!amount || !category)
            return alert("❌ กรุณากรอกจำนวนเงินและเลือกหมวดหมู่");
    ...
    return (
        <div className=" min-h-screen bg-amber-50 pt-16 flex flex-col items-center">
            ...
            <div className="bg-white p-6 rounded-lg shadow-lg mx-4 mt-4 w-full max-w-5xl">
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <input
                        ...
                        className="w-full p-4 text-3xl text-center bg-amber-100 rounded-lg h-16"
                        placeholder="฿0.00"
                    />
                    ...
                    <input
                        ...
                        className="w-full p-4 text-lg bg-amber-100 rounded-lg h-16"
            ...
            {/* แป้นพิมพ์ตัวเลข */}
            <Keypad
                onPress={(key) => setAmount((prev) => prev + key)}
                onDelete={() => setAmount((prev) => prev.slice(0, -1))}
                onSubmit={handleSubmit} // ✅ เพิ่ม onSubmit เพื่อให้กด ✔ บันทึกได้
            />
        </div>
    );
};
